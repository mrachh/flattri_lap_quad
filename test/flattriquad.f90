      ...
      real *8, allocatable :: verts(:,:),targs(:,:),area(:)
      ...
      real *8, allocatable :: uvs_targ(:,:),gradrho(:,:),gradrho2(:,:)
      ...
      allocate(verts(3,nvert),targs(3,ntarg),triind(3,ntri),area(ntri))
      ...
      eps_quad = 1.0d-4
      ntarguse = 1000
      print *, "ntarguse=",ntarguse
      allocate(gradrho(3,ntarg),gradrho2(3,ntarg))
...
!$      t1 = omp_get_wtime()      
      ...
      call lpeval_lap_g(ntarguse,targs,npatches,centers,nvert, &
        verts,triind,area,normals,charges,rfac,nover,eps_quad,gradrho2)
...
!$      t1 = omp_get_wtime()      
      ...
      call lpcomp_grads0(npatches,norders,ixyzs,&
        iptype,npts,srccoefs,srcvals, nvert,verts,triind, &
        ndtarg,ntarguse,targs,ipatch_id, &
        uvs_targ,eps_quad,eps,chargesuse,gradrho)

      call cpu_time(t2)
!$      t2 = omp_get_wtime()      

      call prin2('time taken in lp eval old=*',t2-t1,1)
      errg = 0
      rg = 0
      do i=1,ntarguse
        do j=1,3
          rg = rg + gradrho(j,i)**2
          errg = errg + abs(gradrho(j,i)-gradrho2(j,i))**2
        enddo
      enddo

      errg = sqrt(errg/rg)
      call prin2('error in gradient=*',errg,1)
...
!
      ...
      subroutine getnearquad_magnetostatics_flattri(npatches,norders,ixyzs, &
        iptype,npts,srccoefs,srcvals,nverts,verts,triind,ndtarg,ntarg, &
        targs,ipatch_id, &
        uvs_targ,eps,iquadtype,nnz,row_ptr,col_ind,iquad,rfac0,nquad, &
        wnear)
!
...
!    - triind: integer (3,npatches)
!         vertices corresponding to each patch
!    - ndtarg: integer
!        leading dimension of target array
!    - ntarg: integer
!        number of targets
...
!
      ...
      real *8, intent(in) :: targs(ndtarg,ntarg),uvs_targ(2,ntarg)
      integer, intent(in) :: ipatch_id(ntarg)
      integer, intent(in) :: nverts,triind(3,npatches)
      ...
      real *8, allocatable :: wnear_tmp(:,:)

      integer ndz,ndd,ndi
      integer ipv
      real *8 dpars,r1(3),r2(3),r3(3),normals(3),vtmp(3),wtmp(3),ds
      real *8 r12(3),r13(3)
      integer ipars
      complex *16 zpars
      integer i,ipatch,iqstart,jtarg,l,npols
      real *8 omp_get_wtime,t1,t2,over4pi
...
!        row_ind,iper)
      call cpu_time(t2)
!$       t2 = omp_get_wtime()
      call prin2('time taken in rsc to csc=*',t2-t1,1)
      allocate(wnear_tmp(3,nquad))

!!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(ipatch,npols,r1,r2,r3,normals) &
!!$OMP PRIVATE(i,jtarg,vtmp,iqstart,l)
!      do ipatch=1,npatches
!        npols = ixyzs(ipatch+1)-ixyzs(ipatch)
!        r1(1:3) = verts(1:3,triind(1,ipatch))
!        r2(1:3) = verts(1:3,triind(2,ipatch))
!        r3(1:3) = verts(1:3,triind(3,ipatch))
!        normals(1:3) = srcvals(10:12,ixyzs(ipatch))
!        do i=col_ptr(ipatch),col_ptr(ipatch+1)-1
!          jtarg = row_ind(i)
!          vtmp(1:3) = 0
!          call pot_int2(r1,r2,r3,normals,targs(1,jtarg),vtmp)
!          iqstart = iquad(iper(i))-1
!          do l=1,npols
!            wnear_tmp(1:3,iqstart+l) = vtmp(1:3)/3.0d0
!          enddo
!        enddo
!      enddo
!!$OMP END PARALLEL DO 
...
!$OMP PRIVATE(i,jtarg,vtmp,iqstart,l)
      do jtarg=1,ntarg
        do i=row_ptr(jtarg),row_ptr(jtarg+1)-1
          ...
          iqstart = iquad(i)-1
          do l=1,npols
! extra minus sign to account for changed sign in pot_int2          
            wnear_tmp(1:3,iqstart+l) = vtmp(1:3)/3*over4pi
          enddo
        enddo
      enddo
!$OMP END PARALLEL DO 
...
!$OMP PARALLEL DO DEFAULT(SHARED)
      do i=1,nquad
        wnear(i,1) = wnear_tmp(1,i)
        wnear(i,2) = wnear_tmp(2,i)
        wnear(i,3) = wnear_tmp(3,i)
      enddo
!$OMP END PARALLEL DO      
      ...
      return
      end subroutine getnearquad_magnetostatics_flattri 
!
!
!
!
...
!
      subroutine lpcomp_grads0(npatches,norders,ixyzs,&
        iptype,npts,srccoefs,srcvals,nverts,verts,triind, &
        ndtarg,ntarg,targs,ipatch_id, &
...
!
      ...
      call cpu_time(t1)
!$      t1 = omp_get_wtime()      
      call getnearquad_magnetostatics_flattri(npatches,norders,ixyzs, &
        iptype,npts,srccoefs,srcvals,nverts,verts,triind,ndtarg,ntarg, &
        targs,ipatch_id, &
...
!
      ...
      real *8, allocatable :: sources_orig(:,:)
...
!$OMP PARALLEL DO DEFAULT(SHARED) 
      do i=1,npts
        abc0(i) = rho(i)
        sources_orig(1:3,i) = srcvals(1:3,i)
      enddo
!$OMP END PARALLEL DO
...
!      print *, "before fmm"
      ...
      call cpu_time(t1)
!$      t1 = omp_get_wtime()      
      call lfmm3d_t_c_g(eps,npts,sources_orig,abc0,ntarg,targtmp, &
        pot_aux,grad_aux,ier)

      call cpu_time(t2)
!$      t2 = omp_get_wtime()      

      call prin2('fmm order 1=*',t2-t1,1)
...
!$      t1 = omp_get_wtime()      
      ...
      call cpu_time(t2)
!$      t2 = omp_get_wtime()      
      call prin2('fmm order 3=*',t2-t1,1)
